...
def _is_local_backend() -> bool:
    ...
    machine.  For local backends — the built-in headless Chromium without a
    cloud provider — the user already has full terminal and network access
    on the same machine, so the check adds no security value.
    """
    return _get_cloud_provider() is None
